...
static uint16_t i2s_tx_buf_a[I2S_SAMPLES_NUM * 2] __aligned(4) = {0}; // 2 channels, 16 bits each
static uint16_t i2s_tx_buf_b[I2S_SAMPLES_NUM * 2] __aligned(4) = {0}; // 2 channels, 16 bits each
static uint16_t i2s_rx_buf_a[I2S_SAMPLES_NUM * 2] __aligned(4) = {0}; // 2 channels, 16 bits each
static uint16_t i2s_rx_buf_b[I2S_SAMPLES_NUM * 2] __aligned(4) = {0}; // 2 channels, 16 bits each
...
#define NAME_LEN                          MAX((sizeof(CONFIG_TARGET_BROADCAST_NAME) + 1U), 128U)
...
static const struct bt_audio_codec_cap codec_cap = BT_AUDIO_CODEC_CAP_LC3(
	...
	(BT_AUDIO_CONTEXT_TYPE_CONVERSATIONAL | BT_AUDIO_CONTEXT_TYPE_MEDIA | BT_AUDIO_CONTEXT_TYPE_UNSPECIFIED|BT_AUDIO_CONTEXT_TYPE_NONE));
...
K_MSGQ_DEFINE(recv_pkt_msgq_l, sizeof(struct recv_pkt_info), JITTER_BUFFER_SIZE, 4U);
K_MSGQ_DEFINE(recv_pkt_msgq_r, sizeof(struct recv_pkt_info), JITTER_BUFFER_SIZE, 4U);
			   ...
			   struct net_buf *buf)
{
	...
	while (k_msgq_num_used_get(&recv_pkt_msgq_l) >= JITTER_BUFFER_CHECK &&
	       k_msgq_num_used_get(&recv_pkt_msgq_r) >= JITTER_BUFFER_CHECK) {
		k_msgq_peek(&recv_pkt_msgq_l, &pkt_info_l);
		k_msgq_peek(&recv_pkt_msgq_r, &pkt_info_r);
		if (false) {
		} else if (pkt_info_l.sdu_ref_us > pkt_info_r.sdu_ref_us && (pkt_info_l.sdu_ref_us - pkt_info_r.sdu_ref_us) > 2000) {
...
int main(void)
{
	...
	while (true) {
		...
		if (!IS_ENABLED(CONFIG_SCAN_BACKGROUND)) {
			err = bt_le_scan_stop();
			if (err != 0) {
