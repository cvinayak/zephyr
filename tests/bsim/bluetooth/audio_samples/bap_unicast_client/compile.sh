...
if [ "${BOARD_TS}" == "nrf5340bsim_nrf5340_cpuapp" ]; then
  ...
  app=${CLIENT_TEST} \
    ...
    conf_overlay="${sample}/boards/nrf5340_audio_dk_nrf5340_cpuapp.conf;${sample}/overlay-sequential.conf" \
    exe_name=bs_${BOARD_TS}_${app}_prj_conf_overlay-sequential_conf sysbuild=1 compile
  app=${CLIENT_TEST} \
    sample=${ZEPHYR_BASE}/${CLIENT_SAMPLE} \
    conf_file=${sample}/prj.conf \
    conf_overlay="${sample}/boards/nrf5340_audio_dk_nrf5340_cpuapp.conf;${sample}/overlay-interleaved.conf" \
