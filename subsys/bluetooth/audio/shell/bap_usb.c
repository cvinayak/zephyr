...
static void bap_usb_send_frames_to_usb(void)
{
	...
	if (is_single_channel) {
		LOG_ERR("Mixing SDU with ts %u to stereo", decoded_sdu.ts);
	}
	...
	if ((++cnt % 100) == 0) { /* TODO replace with recv_stats_interval */
			     ...
			     size_t frame_size, uint32_t ts)
{
	...
	const uint8_t ts_jitter_us = 5; /* timestamps may have jitter */
	 ...
	 */
	if (ts + ts_jitter_us < decoded_sdu.ts) {
		/* Old data, discard */
		return -ENOEXEC;
	} else if (ts > decoded_sdu.ts + ts_jitter_us) {
		/* We are getting new data - Send existing data to ring buffer */
		if (decoded_sdu.left_frames_cnt != decoded_sdu.right_frames_cnt) {
			LOG_ERR("Timestamp mismatch between new ts %u and existing ts %u "
				"(decoded_sdu.left_frames_cnt %u "
				"decoded_sdu.right_frames_cnt %u chan_allocation %d)",
				ts, decoded_sdu.ts, decoded_sdu.left_frames_cnt,
				decoded_sdu.right_frames_cnt, chan_allocation);
		}
