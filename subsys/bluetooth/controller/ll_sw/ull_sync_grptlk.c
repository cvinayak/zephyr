...
static struct lll_sync_iso_stream stream_pool[CONFIG_BT_CTLR_SYNC_ISO_STREAM_COUNT];
static struct ll_iso_rx_test_mode test_mode[CONFIG_BT_CTLR_SYNC_ISO_STREAM_COUNT];
static void *stream_free;

uint8_t ll_grptlk_sync_create(uint8_t big_handle, uint16_t sync_handle, uint8_t encryption,
			      uint8_t *bcode, uint8_t mse, uint16_t sync_timeout, uint8_t num_bis,
			      uint8_t *bis)
{
...
uint8_t ull_sync_grptlk_lll_index_get(struct lll_sync_iso *lll)
{
...
struct lll_sync_iso_stream *ull_sync_grptlk_stream_get(uint16_t handle)
{
	...
	sync_iso = ull_sync_grptlk_by_stream_get(handle);
...
struct lll_sync_iso_stream *ull_sync_grptlk_lll_stream_get(uint16_t handle)
{
	return ull_sync_grptlk_stream_get(handle);
}
...
void ull_sync_grptlk_setup(struct ll_sync_iso_set *sync_iso, struct node_rx_pdu *node_rx,
			   uint8_t *acad, uint8_t acad_len)
{
	...
	if ((acad_len < (PDU_BIG_INFO_CLEARTEXT_SIZE + PDU_ADV_DATA_HEADER_SIZE)) ||
	...
	bi_size = acad[PDU_ADV_DATA_HEADER_LEN_OFFSET] - PDU_ADV_DATA_HEADER_TYPE_SIZE;
	bi = (void *)&acad[PDU_ADV_DATA_HEADER_DATA_OFFSET];

	lll = &sync_iso->lll;
	(void)memcpy(lll->seed_access_addr, &bi->seed_access_addr, sizeof(lll->seed_access_addr));
	(void)memcpy(lll->base_crc_init, &bi->base_crc_init, sizeof(lll->base_crc_init));

	(void)memcpy(lll->data_chan_map, bi->chm_phy, sizeof(lll->data_chan_map));
	...
	if (IS_ENABLED(CONFIG_BT_CTLR_BROADCAST_ISO_ENC) && lll->enc &&
	    (bi_size == PDU_BIG_INFO_ENCRYPTED_SIZE)) {
	...
	sync_iso->timeout_reload =
		RADIO_SYNC_EVENTS((sync_iso->timeout * 10U * USEC_PER_MSEC), interval_us);

	sca = sync_iso->sync->lll.sca;
	lll->window_widening_periodic_us = DIV_ROUND_UP(
		((lll_clock_ppm_local_get() + lll_clock_ppm_get(sca)) * interval_us), USEC_PER_SEC);
	...
	sync_iso_offset_us += PDU_BIG_INFO_OFFS_GET(bi) * lll->window_size_event_us;
	/* Skip to first selected BIS subevent */
	stream = ull_sync_grptlk_stream_get(lll->stream_handle[0]);
	if (lll->bis_spacing >= (lll->sub_interval * lll->nse)) {
		sync_iso_offset_us += (stream->bis_index - 1U) * lll->sub_interval *
				      ((lll->irc * lll->bn) + lll->ptc);
	} else {
		sync_iso_offset_us += (stream->bis_index - 1U) * lll->bis_spacing;
	}
	sync_iso_offset_us -= PDU_AC_US(pdu->len, sync_iso->sync->lll.phy, ftr->phy_flags);
	 ...
	 */
	ticks_expire =
		ftr->ticks_anchor - ticks_slot_offset + HAL_TICKER_US_TO_TICKS(sync_iso_offset_us);
	ticks_threshold = ticker_ticks_now_get() + HAL_TICKER_US_TO_TICKS(EVENT_OVERHEAD_START_US);
	ticks_diff = ticker_ticks_diff_get(ticks_expire, ticks_threshold);
	if (ticks_diff & BIT(HAL_TICKER_CNTR_MSBIT)) {
		sync_iso_offset_us += interval_us - lll->window_widening_periodic_us;
	...
	mfy_lll_prepare.fp = lll_sync_grptlk_create_prepare;

	handle = sync_iso_handle_get(sync_iso);
	ret = ticker_start(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_ULL_HIGH,
			   (TICKER_ID_SCAN_SYNC_ISO_BASE + sync_iso_handle_to_index(handle)),
			   ...
			   (sync_iso->ull.ticks_slot + ticks_slot_overhead), ticker_cb, sync_iso,
...
void ull_sync_grptlk_estab_done(struct node_rx_event_done *done)
{
	...
	if (done->extra.trx_cnt || done->extra.estab_failed) {
		...
		mfy_lll_prepare.fp = lll_sync_grptlk_prepare;
...
void ull_sync_grptlk_done(struct node_rx_event_done *done)
{
	...
	if (ticks_drift_plus || ticks_drift_minus || lazy || force) {
		...
		ticker_status = ticker_update(
			...
			(TICKER_ID_SCAN_SYNC_ISO_BASE + sync_iso_handle_to_index(handle)),
			ticks_drift_plus, ticks_drift_minus, 0U, 0U, lazy, force,
...
static void disable(uint8_t sync_idx)
{
	...
	(void)ticker_stop(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_ULL_HIGH,
			  TICKER_ID_SCAN_SYNC_ISO_RESUME_BASE + sync_idx, NULL, NULL);

	err = ull_ticker_stop_with_mark(TICKER_ID_SCAN_SYNC_ISO_BASE + sync_idx, sync_iso,
					&sync_iso->lll);
...
static void timeout_cleanup(struct ll_sync_iso_set *sync_iso)
{
	...
	if (mfy_lll_prepare.fp == lll_sync_grptlk_prepare) {
		rx->hdr.type = NODE_RX_TYPE_SYNC_ISO_LOST;
		      ...
		      uint16_t lazy, uint8_t force, void *param)
{
	...
	if (!IS_ENABLED(CONFIG_BT_TICKER_LOW_LAT) && !IS_ENABLED(CONFIG_BT_CTLR_LOW_LAT) &&
	...
	/* Kick LLL prepare */
	ret = mayfly_enqueue(TICKER_USER_ID_ULL_HIGH, TICKER_USER_ID_LLL, 0U, &mfy_lll_prepare);
	LL_ASSERT(!ret);
...
static void ticker_update_op_cb(uint32_t status, void *param)
{
	LL_ASSERT(status == TICKER_STATUS_SUCCESS || param == ull_disable_mark_get());
}
...
static void lll_flush(void *param)
{
	...
	lll_sync_grptlk_flush(handle, param);
...
static void stop_ticker(struct ll_sync_iso_set *sync_iso, ticker_op_func fp_op_func)
{
	...
	(void)ticker_stop(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_ULL_HIGH,
			  TICKER_ID_SCAN_SYNC_ISO_RESUME_BASE + sync_iso_handle_to_index(handle),
			  NULL, NULL);
