...
static uint32_t cis_offset_calc(const struct ll_conn_iso_stream *cis, uint32_t ticks_offset);
			      ...
			      uint8_t  *access_addr)
{
	...
	if (false) {
		...
		cis_offset = cis_offset_calc(cis, conn->ull.ticks_slot);
				   ...
				   uint16_t *conn_event_count)
{
	...
	*cis_offset_max = (conn->lll.interval * CONN_INT_UNIT_US) -
	...
	/* Calculate the offset for the select CIS in the CIG */
	*cis_offset_min = cis_offset_calc(cis, conn->ull.ticks_slot);
...
static void mfy_cig_offset_get(void *param)
{
	...
	/* Calculate the offset for the select CIS in the CIG */
	offset_min_us = cis_offset_calc(cis, ticks_to_expire);
