...
static uint32_t adv_iso_start(struct ll_adv_iso_set *adv_iso, uint32_t iso_interval_us);
...
static void pdu_big_info_chan_map_phy_set(uint8_t *chm_phy, uint8_t *chan_map, uint8_t phy);
...
static struct lll_adv_iso_stream stream_pool[CONFIG_BT_CTLR_ADV_ISO_STREAM_COUNT];
static void *stream_free;

static uint8_t grptlk_create(uint8_t big_handle, uint8_t adv_handle, uint8_t num_bis,
			     uint32_t sdu_interval, uint16_t max_sdu, uint16_t max_latency,
			     uint8_t rtn, uint8_t phy, uint8_t packing, uint8_t framing,
			     uint8_t encryption, uint8_t *bcode, uint16_t iso_interval, uint8_t nse,
			     uint16_t max_pdu, uint8_t bn, uint8_t irc, uint8_t pto,
			     bool test_config)
{
	...
	/* Check if encryption supported - abort */
	if (encryption) {
		return BT_HCI_ERR_CMD_DISALLOWED;
	};
	...
	/* Check if framed - abort */
	if (framing) {
		return BT_HCI_ERR_CMD_DISALLOWED;
	};

	/* Check if interleaved - abort */
	if (packing) {
		return BT_HCI_ERR_CMD_DISALLOWED;
	};
	...
	if (aux && aux->is_started) {
		...
		time_us = PDU_AC_US(PDU_AC_PAYLOAD_SIZE_MAX, adv->lll.phy_s, adv->lll.phy_flags) +
	...
	if (sync->is_started) {
		...
		time_us = PDU_AC_US(PDU_AC_PAYLOAD_SIZE_MAX, sync->lll.adv->phy_s,
	...
	for (uint8_t i = 0U; i < num_bis; i++) {
		...
		memq_init(stream->link_tx_free, &stream->memq_tx.head, &stream->memq_tx.tail);
		stream->link_tx_free = NULL;

		stream->pkt_seq_num = 0U;

		lll_adv_iso->stream_handle[i] = adv_iso_stream_handle_get(stream);
	}

	lll_adv_iso->max_pdu = MIN(LL_BIS_OCTETS_TX_MAX, max_sdu);
	sdu_per_event = MAX((max_latency * USEC_PER_MSEC / sdu_interval), 2U) - 1U;
	 ...
	 */
	iso_interval_us =
		((sdu_interval * lll_adv_iso->bn * sdu_per_event) / (bn * PERIODIC_INT_UNIT_US)) *
	...
	do {
		...
		latency_packing =
			lll_adv_iso->sub_interval * lll_adv_iso->nse * lll_adv_iso->num_bis;
		event_spacing = latency_packing + ctrl_spacing + EVENT_OVERHEAD_START_US +
	...
	/* Sequential Packing */
	lll_adv_iso->bis_spacing = lll_adv_iso->sub_interval * lll_adv_iso->nse;
	...
	(void)lll_csrand_get(lll_adv_iso->base_crc_init, sizeof(lll_adv_iso->base_crc_init));
	lll_adv_iso->data_chan_count = ull_chan_map_get(lll_adv_iso->data_chan_map);
	...
	bi_ad[PDU_ADV_DATA_HEADER_LEN_OFFSET] =
		pdu_big_info_size + (PDU_ADV_DATA_HEADER_SIZE - PDU_ADV_DATA_HEADER_LEN_SIZE);
	...
	pdu_big_info_chan_map_phy_set(big_info->chm_phy, lll_adv_iso->data_chan_map, phy);
...
uint8_t ll_grptlk_create(uint8_t big_handle, uint8_t adv_handle, uint8_t num_bis,
			 uint32_t sdu_interval, uint16_t max_sdu, uint16_t max_latency, uint8_t rtn,
			 uint8_t phy, uint8_t packing, uint8_t framing, uint8_t encryption,
			 uint8_t *bcode)
{
	return grptlk_create(big_handle, adv_handle, num_bis, sdu_interval, max_sdu, max_latency,
			     rtn, phy, packing, framing, encryption, bcode, 0 /*iso_interval*/,
			     0 /*nse*/, 0 /*max_pdu*/, 0 /*bn*/, 0 /*irc*/, 0 /*pto*/, false);
}
...
void ull_adv_grptlk_done_complete(struct node_rx_event_done *done)
{
	...
	mfy_lll_prepare.fp = lll_adv_grptlk_prepare;
...
void ull_adv_grptlk_done_terminate(struct node_rx_event_done *done)
{
	...
	ret = ticker_stop(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_ULL_HIGH,
			  (TICKER_ID_ADV_ISO_BASE + lll->handle), ticker_stop_op_cb, adv_iso);
	LL_ASSERT((ret == TICKER_STATUS_SUCCESS) || (ret == TICKER_STATUS_BUSY));
...
struct lll_adv_iso_stream *ull_adv_grptlk_lll_stream_get(uint16_t handle)
{
	return ull_adv_grptlk_stream_get(handle);
}
...
void ull_adv_grptlk_stream_release(struct ll_adv_iso_set *adv_iso)
{
	...
	while (lll->num_bis--) {
		...
		link = memq_deinit(&stream->memq_tx.head, &stream->memq_tx.tail);
		LL_ASSERT(link);
...
static uint32_t adv_iso_time_get(const struct ll_adv_iso_set *adv_iso, bool max)
{
	...
	if (IS_ENABLED(CONFIG_BT_CTLR_ADV_ISO_RESERVE_MAX) || max) {
		 ...
		 * packing.
		 */
		time_us = (pdu_spacing * lll_iso->nse * lll_iso->num_bis) + ctrl_spacing;

	} else if (lll_iso->bis_spacing >= (lll_iso->sub_interval * lll_iso->nse)) {
		/* Time reservation omitting PTC subevents in sequetial
		 * packing.
		 */
		time_us = pdu_spacing * ((lll_iso->nse * lll_iso->num_bis) - lll_iso->ptc);

	} else {
		/* Time reservation omitting PTC subevents in interleaved
		 * packing.
		 */
		time_us = pdu_spacing * ((lll_iso->nse - lll_iso->ptc) * lll_iso->num_bis);
	}
...
static uint32_t adv_iso_start(struct ll_adv_iso_set *adv_iso, uint32_t iso_interval_us)
{
	...
	if (!err) {
		ticks_anchor += HAL_TICKER_US_TO_TICKS(MAX(EVENT_MAFS_US, EVENT_OVERHEAD_START_US) -
						       EVENT_OVERHEAD_START_US +
						       (EVENT_TICKER_RES_MARGIN_US << 1));
	}

	/* setup to use ISO create prepare function for first radio event */
	mfy_lll_prepare.fp = lll_adv_grptlk_create_prepare;

	ret_cb = TICKER_STATUS_BUSY;
	ret = ticker_start(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_THREAD,
			   (TICKER_ID_ADV_ISO_BASE + adv_iso->lll.handle), ticks_anchor, 0U,
			   HAL_TICKER_US_TO_TICKS(iso_interval_us),
			   HAL_TICKER_REMAINDER(iso_interval_us), TICKER_NULL_LAZY, ticks_slot,
			   ticker_cb, adv_iso, ull_ticker_status_give, (void *)&ret_cb);
	ret = ull_ticker_status_take(ret, &ret_cb);
...
static void adv_iso_chm_complete_commit(struct lll_adv_iso *lll_iso)
{
	...
	do {
		...
		if (ad_len && (ad[PDU_ADV_DATA_HEADER_TYPE_OFFSET] == BT_DATA_BIG_INFO)) {
			break;
		}
	...
	pdu_big_info_chan_map_phy_set(bi->chm_phy, lll_iso->data_chan_map, lll_iso->phy);
		      ...
		      uint16_t lazy, uint8_t force, void *param)
{
	...
	adv_iso->big_ref_point = isoal_get_wrapped_time_us(
		HAL_TICKER_TICKS_TO_US(ticks_at_expire), (remainder_us + EVENT_OVERHEAD_START_US));
...
static void ticker_stop_op_cb(uint32_t status, void *param)
{
	...
	mfy.param = param;
	ret = mayfly_enqueue(TICKER_USER_ID_ULL_LOW, TICKER_USER_ID_ULL_HIGH, 0U, &mfy);
	LL_ASSERT(!ret);
}
...
static void tx_lll_flush(void *param)
{
	...
	while (num_bis--) {
		...
		link2 = memq_dequeue(stream->memq_tx.tail, &stream->memq_tx.head, (void **)&tx);
		while (link2) {
			tx->next = link2;
