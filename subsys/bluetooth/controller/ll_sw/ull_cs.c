...
/*
 * Weak stubs for LLCP CS functions. Overridden when the LLCP CS module
 * is compiled in a subsequent commit.
 */
__weak uint8_t ull_cp_cs_read_remote_supported_capabilities(struct ll_conn *conn)
{
	ARG_UNUSED(conn);
...
__weak uint8_t ull_cp_cs_read_remote_fae_table(struct ll_conn *conn)
{
	ARG_UNUSED(conn);
...
__weak uint8_t ull_cp_cs_create_config(struct ll_conn *conn, uint8_t config_id,
				       uint8_t action)
{
	ARG_UNUSED(conn);
...
__weak uint8_t ull_cp_cs_security_enable(struct ll_conn *conn)
{
	ARG_UNUSED(conn);
...
__weak uint8_t ull_cp_cs_procedure_enable(struct ll_conn *conn, uint8_t config_id,
					  uint8_t enable)
{
	ARG_UNUSED(conn);
	ARG_UNUSED(config_id);
...
/*
 * Weak stubs for LLL CS functions. Overridden when the nrf LLL CS
 * module is compiled in a subsequent commit.
 */
__weak int lll_cs_init(void)
{
	return 0;
}

__weak int lll_cs_reset(void)
{
	return 0;
}

__weak void lll_cs_prepare(void *param)
{
	ARG_UNUSED(param);
}
