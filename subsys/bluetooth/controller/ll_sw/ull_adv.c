...
static ULL_HIGH_CODE_RAM_ATTR void adv_disable(void *param);
...
uint8_t ll_adv_set_hci_handle_get(uint8_t handle)
{
	...
	LL_ASSERT_DBG(adv && adv->is_created);
...
static void disabled_cb(void *param)
{
	...
	LL_ASSERT_DBG(adv->node_rx_cc_free);
...
static inline uint8_t *adv_pdu_adva_get(struct pdu_adv *pdu)
{
	...
	if (pdu->type == PDU_ADV_TYPE_EXT_IND) {
		LL_ASSERT_DBG(hdr_flags.adv_addr);
