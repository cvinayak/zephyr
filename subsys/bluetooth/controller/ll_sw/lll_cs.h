...
struct lll_cs {
	...
	struct lll_cs_iq_sample pct_scratch[1];
...
/* LLL prepare pipeline entry point for a Channel Sounding event.
 *
 * Invoked as a mayfly from the ULL ticker callback (see ull_cs.c) with the
 * struct lll_prepare_param built for the event. It enables the high frequency
 * clock and registers the CS prepare/abort callbacks with the LLL preemption
 * pipeline, mirroring the ISO LLL prepare entry points.
 */
void lll_cs_prepare(void *param);

/* Program the radio Channel Sounding hardware for the given step. */
void lll_cs_step_setup(const struct lll_cs_step *step);

/* Return the RTT timestamp captured for the last received CS_SYNC packet of the
 * current step, in radio time-base ticks.
 */
uint32_t lll_cs_step_rtt_get(void);

/* Copy up to max captured PCT samples of the current step into samples and
