...
/* Preempt timeout timer HAL abstraction.
 *
 * Single timer design:
 *   Uses NRF_TIMER0 as a dedicated one-shot preempt timer. NRF_TIMER0 is
 *   free when BT_CTLR_SW_SWITCH_SINGLE_TIMER is selected (EVENT_TIMER =
 *   NRF_TIMER4) or on nRF54L/H SoCs where EVENT_TIMER = NRF_TIMER10.
 *
 * Dual timer design:
 *   Uses SW_SWITCH_TIMER (NRF_TIMER1) with an additional CC channel. The
 *   preempt compare channel is beyond the ones used for tIFS switching.
 */
#if defined(CONFIG_BT_CTLR_PREEMPT_TIMEOUT_TIMER_SINGLE)
#define HAL_PREEMPT_TIMER               NRF_TIMER0
#define HAL_PREEMPT_TIMER_CC_OFFSET     0U
#define HAL_PREEMPT_TIMER_EVENT         NRF_TIMER_EVENT_COMPARE0
#define HAL_PREEMPT_TIMER_INT_MASK      TIMER_INTENSET_COMPARE0_Msk
#define HAL_PREEMPT_TIMER_PRESCALER     HAL_EVENT_TIMER_PRESCALER_VALUE

/* IRQ name defaults to TIMER0_IRQn; overridden per SoC if needed (e.g., nRF54L). */
#if !defined(HAL_PREEMPT_TIMER_IRQn)
#define HAL_PREEMPT_TIMER_IRQn          TIMER0_IRQn
#endif /* !HAL_PREEMPT_TIMER_IRQn */

#elif defined(CONFIG_BT_CTLR_PREEMPT_TIMEOUT_TIMER_DUAL)
/* Use SW_SWITCH_TIMER (NRF_TIMER1) with a dedicated CC channel for preempt. */
#define HAL_PREEMPT_TIMER               SW_SWITCH_TIMER
#define HAL_PREEMPT_TIMER_IRQn          TIMER1_IRQn

/* Select the CC channel beyond the ones reserved for tIFS SW switching.
 * Without PHY_CODED: SW_SWITCH_TIMER uses CC0, CC1 → use CC2.
 * With PHY_CODED:    SW_SWITCH_TIMER uses CC0..CC3 → use CC4 (nRF53 only,
 *                    which has 8 CC channels on TIMER1).
 */
#if defined(CONFIG_BT_CTLR_PHY_CODED)
#define HAL_PREEMPT_TIMER_CC_OFFSET     (SW_SWITCH_TIMER_EVTS_COMP_S2_BASE + 2U)
#else
#define HAL_PREEMPT_TIMER_CC_OFFSET     (SW_SWITCH_TIMER_EVTS_COMP_BASE + 2U)
#endif /* CONFIG_BT_CTLR_PHY_CODED */

#define HAL_PREEMPT_TIMER_EVENT         nrf_timer_compare_event_get(HAL_PREEMPT_TIMER_CC_OFFSET)
#define HAL_PREEMPT_TIMER_INT_MASK      nrf_timer_compare_int_get(HAL_PREEMPT_TIMER_CC_OFFSET)
