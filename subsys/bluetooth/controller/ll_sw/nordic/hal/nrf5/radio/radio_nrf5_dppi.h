...
static inline void hal_radio_nrf_ppi_channels_disable(uint32_t mask)
{
	...
	/* FIXME: call `nrf_dppi_channels_disable(NRF_DPPIC00, mask_dppic00);` , where
	 *        mask_dppic00 is translated bitmap of channels in DPPIC00 that corresponds to
	 *        mask value of DPPIC in radio domain
	 */
...
static inline void hal_event_timer_start_ppi_config(void)
{
	...
	/* Setup TIMER start subscribe in main domain */
	nrf_timer_subscribe_set(EVENT_TIMER, NRF_TIMER_TASK_START,
				HAL_NRF54LX_DUAL_TIMER_EVENT_TIMER_START_DPPIC00);

#else /* CONFIG_BT_CTLR_SW_SWITCH_SINGLE_TIMER */
	/* Setup TIMER start subscribe in radio domain */
