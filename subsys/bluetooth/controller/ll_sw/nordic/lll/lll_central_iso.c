...
static int prepare_cb(struct lll_prepare_param *p)
{
	...
	if (IS_ENABLED(CONFIG_BT_CTLR_CONN_ISO_INTERLEAVED) && (cig_lll->packing != 0U)) {
...
static void isr_prepare_subevent(void *param)
{
	...
	} else if (IS_ENABLED(CONFIG_BT_CTLR_CONN_ISO_INTERLEAVED) &&
		   (ull_conn_iso_lll_group_get_by_stream(cis_lll)->packing != 0U)) {
