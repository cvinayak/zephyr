...
static LLL_ISR_CODE_RAM_ATTR void isr_window(void *param);
...
static LLL_ISR_CODE_RAM_ATTR void isr_abort(void *param);
	...
	* (EVENT_OVERHEAD_PREEMPT_US <= EVENT_OVERHEAD_PREEMPT_MIN_US)
	*/
static LLL_ISR_CODE_RAM_ATTR void isr_done_cleanup(void *param);
...
static LLL_ISR_CODE_RAM_ATTR void isr_common_done(void *param)
{
	struct node_rx_pdu *node_rx;
	struct lll_scan *lll;
