...
static int prepare_cb_common(struct lll_prepare_param *p)
{
	...
	if (p->ticks_drift != 0U) {
		...
		hal_ticker_remove_jitter(&ticks_at_event, &remainder_us);
		drift_us += HAL_TICKER_TICKS_TO_US(p->ticks_at_expire - ticks_at_event) -
			    remainder_us;
	...
	if (false) {
		...
		if ((p->ticks_drift != 0U) && (skipped_bis < (lll->num_bis - 1U))) {
			...
			ret = ticker_start(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_LLL,
					   ...
					   ticks_at_event, HAL_TICKER_US_TO_TICKS(bis_offset),
...
static int is_abort_cb(void *next, void *curr, lll_prepare_cb_t *resume_cb)
{
	...
	if (next == NULL) {
		 ...
		 * -EAGAIN so that this ready event is placed as resume event.
		 */
...
static void ticker_resume_cb(uint32_t ticks_at_expire, uint32_t ticks_drift, uint32_t remainder,
			     uint16_t lazy, uint8_t force, void *param)
{
	/* TODO: Setup resume place holder for next BIS spacing */
}
