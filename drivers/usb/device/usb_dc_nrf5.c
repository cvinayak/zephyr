 ...
 *
 * nRF52840 USB device controller driver. The driver implements the low
 * level control routines to deal directly with the nRF52840 USBD peripheral.
 */
...
#define SYS_LOG_DOMAIN "USBD"
#include <logging/sys_log.h>

/**
 * TODO:
 * Find alternatives for SET_BIT and friends
 * For now, define them here
 */
#define SET_BIT(W,B)  ((W) |= (u32_t)(1U << (B)))
#define CLR_BIT(W, B) ((W) &= (~((u32_t)1 << (B))))
#define IS_SET(W,B) (((W) >> (B)) & 1)

/* TODO: Find the right place for these function prototypes */
void nrf5_power_clock_install_usb_cb(void (*cb)(nrf_power_event_t ev));
void nrf5_power_clock_remove_usb_cb(void);

#define NRF_CTRL_EPIN_COUNT	1UL
#define NRF_CTRL_EPOUT_COUNT	1UL
#define NRF_DATA_EPIN_COUNT	7UL
#define NRF_DATA_EPOUT_COUNT	7UL
#define NRF_ISO_EPIN_COUNT	1UL
#define NRF_ISO_EPOUT_COUNT	1UL
 ...
 * be based derived from the theoretical number of backlog events possible
 * depending on the number of endpoints configured.
 */
...
#define CFG_EPIN_CNT (CONFIG_USBD_NRF52840_EPIN_COUNT + NRF_CTRL_EPIN_COUNT)

/** Number of OUT Endpoints configured (including control) */
#define CFG_EPOUT_CNT (CONFIG_USBD_NRF52840_EPOUT_COUNT + NRF_CTRL_EPOUT_COUNT)

/** Number of ISO IN Endpoints */
#ifdef CONFIG_USBD_NRF52840_ISOIN
#define CFG_EP_ISOIN_CNT NRF_ISO_EPIN_COUNT
#else
#define CFG_EP_ISOIN_CNT 0
#endif

/** Number of ISO OUT Endpoints */
#ifdef CONFIG_USBD_NRF52840_ISOOUT
#define CFG_EP_ISOOUT_CNT NRF_ISO_EPOUT_COUNT
#else
#define CFG_EP_ISOOUT_CNT 0
#endif
...
static inline bool nrf5_usbd_ep_is_valid(const u8_t ep)
{
	u8_t ep_num = NRF_USBD_EP_NR_GET(ep);

	if (NRF_USBD_EPIN_CHECK(ep)) {
		/* TODO: Seems redundant. Is this necessary? */
		if (ep_num >= NRF_USBD_EPIN_CNT) {
			return false;
		}
...
static struct nrf5_usbd_ep_ctx *nrf5_usbd_endpoint_ctx(const u8_t ep)
{
	struct nrf5_usbd_ctx *ctx;
	u8_t ep_num;

	/* TODO: Use ep_ctx variable */

	if (!nrf5_usbd_ep_is_valid(ep))
		return NULL;
...
static struct nrf5_usbd_ep_ctx *nrf5_usbd_in_endpoint_ctx(const u8_t ep)
{
	return nrf5_usbd_endpoint_ctx(NRF_USBD_EPIN(ep));
}

static struct nrf5_usbd_ep_ctx *nrf5_usbd_out_endpoint_ctx(const u8_t ep)
{
	return nrf5_usbd_endpoint_ctx(NRF_USBD_EPOUT(ep));
}

static void nrf5_usbd_cfg_ep_interrupt(const u8_t ep, bool set)
{
	...
	irq_disable(NRF52840_IRQ_USBD_IRQn);

	if (set) {
		mask |= NRF_USBD_INT_STARTED_MASK;
		mask |= nrf_usbd_int_enable_get();
	...
	irq_enable(NRF52840_IRQ_USBD_IRQn);
}

static struct nrf5_usbd_ep_ctx *nrf5_usbd_epstatus_to_ep_ctx(u32_t epstatus)
{
	int pos;
	u8_t ep = 0;
	...
	if (epstatus & USBD_EPSTATUS_EPIN_MASK) {
		...
		while (pos < USBD_EPSTATUS_EPIN8_Pos) {
			if (epstatus & BIT(pos)) {
				...
				return nrf5_usbd_endpoint_ctx(ep);
			}
			pos++;
		}
	}
	...
	if (epstatus & USBD_EPSTATUS_EPOUT_MASK) {
		...
		while (pos < USBD_EPSTATUS_EPOUT8_Pos) {
			if (epstatus & BIT(pos)) {
				ep = NRF_USBD_EPOUT(pos - USBD_EPSTATUS_EPOUT0_Pos);
				return nrf5_usbd_endpoint_ctx(ep);
			}
			pos++;
		}
...
static struct nrf5_usbd_ep_ctx *
nrf5_usbd_epdatastatus_to_ep_ctx(u32_t epdatastatus)
{
	int pos;
	u8_t ep = 0;
...
static void nrf5_usbd_start_epin_task(u8_t ep)
{
...
static void nrf5_usbd_start_epout_task(u8_t ep)
{
	u8_t epnum = NRF_USBD_EP_NR_GET(ep);
	nrf_usbd_task_t task;
...
static void nrf5_usbd_start_ep0rcvout_task(void)
{
	nrf_usbd_task_trigger(NRF_USBD_TASK_EP0RCVOUT);
}

static void nrf5_usbd_start_ep0status_task(void)
{
	nrf_usbd_task_trigger(NRF_USBD_TASK_EP0STATUS);
}

static void nrf5_usbd_start_ep0stall_task(void)
{
	nrf_usbd_task_trigger(NRF_USBD_TASK_EP0STALL);
}

static inline struct ep_usb_event *nrf5_usbd_alloc_ep_usb_event(void)
{
	int ret;
...
static inline void nrf5_usbd_free_ep_usb_event(struct ep_usb_event *ev)
{
	k_mem_pool_free(&ev->block);
}

static inline void nrf5_usbd_enqueue_ep_usb_event(struct ep_usb_event *ev)
{
	k_fifo_put(&get_usbd_ctx()->work_queue, ev);
}

static inline struct ep_usb_event *nrf5_usbd_dequeue_ep_usb_event(void)
{
	return k_fifo_get(&get_usbd_ctx()->work_queue, K_NO_WAIT);
}

static inline void nrf5_usbd_flush_ep_usb_events(void)
{
	struct ep_usb_event *ev;

	do {
		ev = nrf5_usbd_dequeue_ep_usb_event();
		if (ev) {
			nrf5_usbd_free_ep_usb_event(ev);
		}
	} while (ev != NULL);
}
...
static u32_t event_map[USBD_INT_CNT] = {
...
static void nrf5_usbd_usb_reset_handler(u32_t pos)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
	...
	SET_BIT(ctx->flags, NRF5_USB_STATUS_CHANGE);
}

static void nrf5_usbd_usb_started_handler(u32_t pos)
{
	...
	ev = nrf5_usbd_alloc_ep_usb_event();
	ev->ep = nrf5_usbd_epstatus_to_ep_ctx(epstatus);
	ev->evt = EP_DMA_START;

	nrf5_usbd_enqueue_ep_usb_event(ev);
}

static void nrf5_usbd_end_epin_handler(u32_t pos)
{
	struct ep_usb_event *ev;
	u8_t ep;
	...
	ev = nrf5_usbd_alloc_ep_usb_event();
	ev->ep = nrf5_usbd_endpoint_ctx(ep);
	ev->evt = EP_DMA_END;

	/* TODO: Should we give the sem here instead?? */

	nrf5_usbd_enqueue_ep_usb_event(ev);
}

static void nrf5_usbd_ep0_datadone_handler(u32_t pos)
{
	struct ep_usb_event *ev;
...
static void nrf5_usbd_end_isoin_handler(u32_t pos)
{
	struct ep_usb_event *ev;
	u8_t ep;
	...
	/* TODO: Should we give back the DMA sem here instead?? */

	nrf5_usbd_enqueue_ep_usb_event(ev);
}

static void nrf5_usbd_end_epout_handler(u32_t pos)
{
	struct ep_usb_event *ev;
	u8_t ep;
	...
	/* TODO: Should we give back the DMA sem here instead?? */
	SET_BIT(ev->misc_u.flags, EP_OUT_DATA_RCVD);

	nrf5_usbd_enqueue_ep_usb_event(ev);
}

static void nrf5_usbd_end_isoout_handler(u32_t pos)
{
	struct ep_usb_event *ev;
	u8_t ep;
...
static void nrf5_usbd_sof_handler(u32_t pos)
{
	struct ep_usb_event *ev;
...
static void nrf5_usbd_usb_event_handler(u32_t pos)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
	u32_t eventcause;
...
static void nrf5_usbd_ep0setup_handler(u32_t pos)
{
	...
	ep_ctx = nrf5_usbd_endpoint_ctx(ep);
	...
	if (REQTYPE_GET_DIR(setup->bmRequestType) == REQTYPE_DIR_TO_DEVICE) {
		if (setup->wLength) {
			SET_BIT(ev->misc_u.flags, EP_CONTROL_WRITE);
		} else {
			SET_BIT(ev->misc_u.flags, EP_CONTROL_WRITE_NO_DATA);
		}
	} else {
		SET_BIT(ev->misc_u.flags, EP_CONTROL_READ);
	}

	nrf5_usbd_enqueue_ep_usb_event(ev);
}

static void nrf5_usbd_epdata_handler(u32_t pos)
{
	struct ep_usb_event *ev;
	u32_t epdatastatus;
	...
	ev = nrf5_usbd_alloc_ep_usb_event();
	ev->ep = nrf5_usbd_epdatastatus_to_ep_ctx(epdatastatus);
...
static void nrf5_usbd_access_fault_handler(u32_t pos)
{
	ARG_UNUSED(pos);
...
static isr_event_handler_t isr_event_handlers[USBD_INT_CNT] = {
	nrf5_usbd_usb_reset_handler,
	nrf5_usbd_usb_started_handler,
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_end_epin_handler,
	...
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_end_epin_handler,
	nrf5_usbd_ep0_datadone_handler,
	nrf5_usbd_end_isoin_handler,
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_epout_handler,
	...
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_epout_handler,
	nrf5_usbd_end_isoout_handler,
	nrf5_usbd_sof_handler,
	nrf5_usbd_usb_event_handler,
	nrf5_usbd_ep0setup_handler,
	nrf5_usbd_epdata_handler,
	nrf5_usbd_access_fault_handler,
};

/** Process interrupts that are enabled */
static u32_t nrf5_usbd_process_interrupts(u32_t mask)
{
 ...
 * @return N/A
 */
static void nrf5_usbd_isr_handler(void *arg)
{
	...
	if (nrf5_usbd_process_interrupts(ctx->enable_mask)) {
 ...
 * @return N/A
 */
static void nrf5_usbd_install_isr(void)
{
	IRQ_CONNECT(NRF52840_IRQ_USBD_IRQn,
		    CONFIG_USBD_NRF52840_IRQ_PRI,
		    nrf5_usbd_isr_handler, &usbd_ctx, 0);
}

static void nrf5_usbd_enable_interrupts(void)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
...
static void nrf5_usbd_disable_interrupts(void)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
...
static void nrf5_usbd_power_event_callback(nrf_power_event_t event)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
	...
	SET_BIT(ctx->flags, NRF5_USB_STATE_CHANGE);
	k_work_submit(&ctx->usb_work);
}
 ...
 */
static int nrf5_usbd_hf_clock_enable(bool on, bool blocking)
{
	int ret = -ENODEV;
	struct device *clock;
...
static void nrf5_usbd_enable_endpoints(struct nrf5_usbd_ctx *ctx)
{
	struct nrf5_usbd_ep_ctx *ep_ctx;
	int i;

	for (i = 0; i < NRF_USBD_EPIN_CNT; i++) {
		ep_ctx = nrf5_usbd_in_endpoint_ctx(i);
		__ASSERT_NO_MSG(ep_ctx);

		if (ep_ctx->cfg.en) {
			nrf_usbd_ep_enable(ep_ctx->cfg.addr);
			nrf5_usbd_cfg_ep_interrupt(ep_ctx->cfg.addr, true);
		}
	}

	for (i = 0; i < NRF_USBD_EPOUT_CNT; i++) {
		ep_ctx = nrf5_usbd_out_endpoint_ctx(i);
		__ASSERT_NO_MSG(ep_ctx);
...
static void nrf5_usbd_handle_state_change(struct nrf5_usbd_ctx *ctx)
{
	switch (ctx->state) {
	case USBD_ATTACHED:
	...
	case USBD_POWERED:
		...
		SET_BIT(ctx->flags, NRF5_USB_STATUS_CHANGE);
		nrf5_usbd_enable_endpoints(ctx);
...
static void nrf5_usbd_handle_status_change(struct nrf5_usbd_ctx *ctx)
{
	if (ctx->status_cb) {
...
static inline void
nrf5_usbd_handle_ctrl_ep_idle_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;

	switch (ev->evt) {
	case EP_SETUP_RECV:
		if (IS_SET(ev->misc_u.flags, EP_CONTROL_READ)) {
			...
			epin0 = nrf5_usbd_endpoint_ctx(NRF_USBD_EPIN(0));
			epin0->state = EP_DATA;
		} else if (IS_SET(ev->misc_u.flags, EP_CONTROL_WRITE)) {
			ep_ctx->state = EP_SETUP;

			/* Initiate reception of EP0 OUT DATA to USBD local buffer */
			nrf5_usbd_start_ep0rcvout_task();
		} else if (IS_SET(ev->misc_u.flags, EP_CONTROL_WRITE_NO_DATA)) {
...
static inline void
nrf5_usbd_handle_ctrl_ep_setup_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
	...
	case EP_DATA_RECV:
	{
		...
		nrf5_usbd_start_epout_task(ep_ctx->cfg.addr);
...
static inline void
nrf5_usbd_handle_ctrl_ep_data_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ctx *ctx = get_usbd_ctx();
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
	...
	case EP_DMA_END:
		 ...
		 */
		if (IS_SET(ev->misc_u.flags, EP_OUT_DATA_RCVD)) {
			if (ep_ctx->buf.len < ep_ctx->cfg.max_sz) {
				/**
				 * ZLP or short packet? Initiate STATUS stage
				 * TODO: Should we do this here or during read()?
				 */
				nrf5_usbd_start_ep0status_task();
				ep_ctx->state = EP_IDLE;
			} else {
				/* We still have some more OUT data to receive */
...
static void nrf5_usbd_handle_ctrl_ep_event(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;

	switch (ep_ctx->state) {
	case EP_IDLE:
		nrf5_usbd_handle_ctrl_ep_idle_state_events(ev);
		break;
	case EP_SETUP:
		nrf5_usbd_handle_ctrl_ep_setup_state_events(ev);
		break;
	case EP_DATA:
		nrf5_usbd_handle_ctrl_ep_data_state_events(ev);
		break;
	case EP_STATUS:
		/**
		 * TODO: HW doesn't indicate STATUS stage completion to SW.
		 * Do we event need STATUS state?
		 */
		break;
	}
}
...
static inline void
nrf5_usbd_handle_data_ep_idle_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
...
static inline void
nrf5_usbd_handle_data_ep_data_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
	...
	case EP_DMA_END:
	{
		...
		if (NRF_USBD_EPIN_CHECK(ep_ctx->cfg.addr)) {
			SET_BIT(ep_ctx->flags, EP_WRITE_PENDING);
		}
		k_sem_give(&ctx->dma_in_use);
	...
	case EP_WRITE_COMPLETE:
	{
		CLR_BIT(ep_ctx->flags, EP_WRITE_PENDING);
...
static void nrf5_usbd_handle_data_ep_event(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;

	switch (ep_ctx->state) {
	case EP_IDLE:
		nrf5_usbd_handle_data_ep_idle_state_events(ev);
		break;
	case EP_DATA:
		nrf5_usbd_handle_data_ep_data_state_events(ev);
		break;
	case EP_SETUP:
	case EP_STATUS:
...
static inline void
nrf5_usbd_handle_iso_ep_idle_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
	...
	case EP_SOF:
	{
		...
		if (NRF_USBD_EPOUT_CHECK(ep_ctx->cfg.addr)) {
			/**
			 * TODO: We should check the ISOCRC USB flag here and discard
			 * the whole data after DMA if need be. USBD will transfer the
			 * OUT data anyway if DMA is set up.
			 */

			/** Have anything in the ISO OUT buffer to write to USB bus? */
			if (ep_ctx->buf.len) {
				nrf_usbd_ep_easydma_set(ep_ctx->cfg.addr,
							(u32_t)ep_ctx->buf.data,
							ep_ctx->cfg.max_sz);
			...
			if (!ep_ctx->buf.len) {
				...
				/**
				 * TODO: What if we are stuck in this work handler forever?
				 * Because, a DMA done event would be waiting in the FIFO forever
				 * Should we instead give the sem back in ISR?
				 */
				k_sem_take(&ctx->dma_in_use, K_FOREVER);
				nrf5_usbd_start_epin_task(ep_ctx->cfg.addr);
			}
		}
	}
		break;
...
static inline void
nrf5_usbd_handle_iso_ep_data_state_events(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
...
static void nrf5_usbd_handle_iso_ep_event(struct ep_usb_event *ev)
{
	struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;
	...
	switch (ep_ctx->state) {
	case EP_IDLE:
		nrf5_usbd_handle_iso_ep_idle_state_events(ev);
		break;
	case EP_DATA:
		nrf5_usbd_handle_iso_ep_data_state_events(ev);
		break;
	case EP_SETUP:
	case EP_STATUS:
...
/* Work handler */
static void nrf5_usbd_work(struct k_work *item)
{
	struct nrf5_usbd_ctx *ctx;
	struct ep_usb_event *ev;
	...
	if (ctx->flags) {
		...
		if (IS_SET(ctx->flags, NRF5_USB_STATE_CHANGE)) {
			nrf5_usbd_handle_state_change(ctx);
			CLR_BIT(ctx->flags, NRF5_USB_STATE_CHANGE);
		}

		if (IS_SET(ctx->flags, NRF5_USB_STATUS_CHANGE)) {
			nrf5_usbd_handle_status_change(ctx);
			CLR_BIT(ctx->flags, NRF5_USB_STATUS_CHANGE);
		}
	...
	while ((ev = nrf5_usbd_dequeue_ep_usb_event()) != NULL) {
		struct nrf5_usbd_ep_ctx *ep_ctx = ev->ep;

		if (ep_ctx) {
			switch (ep_ctx->cfg.type) {
			case USB_DC_EP_CONTROL:
				nrf5_usbd_handle_ctrl_ep_event(ev);
				break;
			case USB_DC_EP_BULK:
			case USB_DC_EP_INTERRUPT:
				nrf5_usbd_handle_data_ep_event(ev);
				break;
			case USB_DC_EP_ISOCHRONOUS:
				nrf5_usbd_handle_iso_ep_event(ev);
				break;
			}
		}
...
static inline bool nrf5_usbd_dev_attached(void)
{
	return get_usbd_ctx()->attached;
}

static inline bool nrf5_usbd_dev_ready(void)
{
	return get_usbd_ctx()->ready;
}

static void nrf5_usbd_endpoint_ctx_init(void)
{
	struct nrf5_usbd_ep_ctx *ep_ctx;
	int ret;
	u32_t i;
	...
	if (CFG_EP_ISOIN_CNT) {
		ep_ctx = nrf5_usbd_in_endpoint_ctx(NRF_USBD_EPIN(8));
		__ASSERT_NO_MSG(ep_ctx);
	...
	if (CFG_EP_ISOOUT_CNT) {
		ep_ctx = nrf5_usbd_out_endpoint_ctx(NRF_USBD_EPOUT(8));
		__ASSERT_NO_MSG(ep_ctx);
...
static void nrf5_usbd_endpoint_ctx_deinit(void)
{
	struct nrf5_usbd_ep_ctx *ep_ctx;
	u32_t i;
...
int usb_dc_attach(void)
{
	...
	k_work_init(&ctx->usb_work, nrf5_usbd_work);
	k_fifo_init(&ctx->work_queue);
	k_sem_init(&ctx->dma_in_use, 1, 1);

	nrf5_power_clock_install_usb_cb(&nrf5_usbd_power_event_callback);

	nrf5_usbd_install_isr();
	nrf5_usbd_enable_interrupts();

	ret = nrf5_usbd_hf_clock_enable(true, false);
	if (ret) {
		goto err_clk_enable;
	}

	nrf5_usbd_endpoint_ctx_init();
	ctx->attached = true;

	return ret;

err_clk_enable:
	nrf5_usbd_disable_interrupts();
...
int usb_dc_detach(void)
{
	...
	ret = nrf5_usbd_hf_clock_enable(false, false);
	if (ret) {
		goto err_clk_disable;
	}

	nrf5_power_clock_remove_usb_cb();

	ctx->flags = 0;
	ctx->state = USBD_DETACHED;
	ctx->status_code = USB_DC_UNKNOWN;

	nrf5_usbd_flush_ep_usb_events();
	k_sem_reset(&ctx->dma_in_use);
	nrf5_usbd_endpoint_ctx_deinit();
...
{
	if (!nrf5_usbd_dev_attached() || !nrf5_usbd_dev_ready()) {
		return -ENODEV;
	}
...
int usb_dc_ep_configure(const struct usb_dc_ep_cfg_data * const ep_cfg)
{
	...
	if (!nrf5_usbd_dev_attached()) {
		return -ENODEV;
	}

	ep_ctx = nrf5_usbd_endpoint_ctx(ep_cfg->ep_addr);
	if (!ep_ctx) {
		return -EINVAL;
	}
...
int usb_dc_ep_enable(const u8_t ep)
{
	...
	if (nrf5_usbd_dev_ready()) {
		nrf_usbd_ep_enable(ep);
		nrf5_usbd_cfg_ep_interrupt(ep, true);
...
int usb_dc_ep_disable(const u8_t ep)
{
	...
	nrf5_usbd_cfg_ep_interrupt(ep, false);
		    ...
		    const u32_t data_len, u32_t * const ret_bytes)
{
	 ...
	 */
	if (IS_SET(ep_ctx->flags, EP_WRITE_PENDING)) {
		return -EAGAIN;
	}
